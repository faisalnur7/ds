    ...
    <div class="space-y-6">
        <section class="rounded-[2rem] ccims-panel-strong p-6 sm:p-8">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div class="max-w-2xl">
                    <p class="text-sm font-medium uppercase tracking-[0.3em] text-amber-200/80">Module</p>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-white/10">
                    ...
                    <tbody class="divide-y divide-white/10">
                        @forelse ($items as $item)
                            <tr class="bg-white/0">
                                <td class="px-4 py-4 font-medium text-white">
                                    {{ data_get($item, 'name') ?? data_get($item, 'member_code') ?? data_get($item, 'action') ?? data_get($item, 'key') ?? data_get($item, 'reference_no') ?? data_get($item, 'status') }}
                                </td>
                                <td class="px-4 py-4 text-sm text-slate-300">
                                    {{ data_get($item, 'description') ?? data_get($item, 'full_name') ?? data_get($item, 'action') ?? data_get($item, 'value_type') ?? data_get($item, 'status') }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="px-4 py-8 text-center text-sm text-slate-400">No records found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
