...
@section('content')
    <div class="space-y-6">
        <section class="rounded-[2rem] ccims-panel-strong p-6 sm:p-8">
            <p class="text-sm font-medium uppercase tracking-[0.3em] text-amber-200/80">{{ __('Compliance') }}</p>
            <h2 class="mt-3 font-[family-name:Space_Grotesk] text-3xl font-bold text-white">{{ __('Audit Logs') }}</h2>
            <p class="mt-3 text-sm leading-6 text-slate-400">{{ __('Read-only history of important system changes.') }}</p>
        </section>
                    ...
                    <tbody class="divide-y divide-white/10">
                        @forelse ($records as $record)
                            <tr>
                                ...
                                <td class="px-4 py-4 text-sm text-slate-300">{{ $record->user?->name ?? __('System') }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-8 text-center text-sm text-slate-400">{{ __('No audit records found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
