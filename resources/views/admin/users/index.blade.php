...
@section('header', __('User Registry'))

@section('content')
    <div class="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-2xl shadow-black/20 backdrop-blur">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="font-[family-name:Space_Grotesk] text-2xl font-bold text-white">{{ __('Users') }}</h2>
                <p class="mt-1 text-sm text-slate-400">{{ __('Manage accounts from a clean, responsive registry.') }}</p>
            </div>
            <div class="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-300">
                {{ $users->total() }} {{ __('total records') }}
            </div>
        </div>
        ...
        <div class="mt-6 overflow-hidden rounded-2xl border border-white/10">
            <table class="min-w-full divide-y divide-white/10">
                <thead class="bg-white/5">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ __('Name') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ __('Email') }}</th>
